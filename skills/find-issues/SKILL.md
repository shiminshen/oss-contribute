...
If the PR is closed-and-unmerged, do NOT silently treat the issue as ripe — inspect *why* it closed. Batch this across every closed-not-merged hit turned up in this pass (one message, one `gh pr view` call per PR, in parallel — same discipline as the token queries above, not one at a time): `gh pr view <pr> --repo <o>/<r> --json state,mergedAt,author,closedAt,comments,reviews,labels`:

- **Approach rejected by a maintainer** (review comment explaining a different design, or "we don't want this") → the issue is design-sensitive; demote to issue-comment-only or drop. A fresh PR with the same approach will be rejected too. (`contribute-upstream`'s Phase 1 step 0c runs a stricter multi-sample version of this same signal once a repo is committed to — this is the cheap single-observation triage for shortlisting.)
- **Auto-closed by an anti-AI-PR bot.** A bot/`github-actions` comment like *"labeled `maybe automated` because it appears to have been fully generated by AI"* or any "confirm you are a real human / disclose if automated" gate means the repo **auto-kills AI-generated PRs** — the same bot-signal family as the account-suffix check above and the merge-author gate below, just keyed on comment wording instead of account name. This is an anti-agent policy (same family as immich/transformers/playwright) — it is **near-fatal for this flow regardless of issue ripeness**. Drop the candidate AND surface the *repo* as a skip-list addition (anti-AI-PR auto-close). The issue may still be technically ripe for a human, but not for a Claude-driven PR.
...
- **Doesn't clearly fit any of the above** (e.g. closed as duplicate-of-another-issue, superseded by a different PR, generic stale-bot close unrelated to AI) → default to caution and treat like "rejected" (demote or drop). Only file a closure under "abandoned" when that reading is unambiguous.
...
If a token query or the timeline returns an open or merged PR, or a closed-not-merged PR that falls in the "rejected", "anti-AI-bot", or default-to-caution buckets above, drop the candidate.
...
The GitHub search API enforces a secondary rate limit that triggers after ~10–30 rapid queries. Trending mode fans out across 7+ repos × {repo metadata, merged-author search, fast-close sample}, so the budget evaporates quickest there — but the same check applies to any batch that fans out across many candidates: the Phase 3 dup-PR token search, the closed-PR `gh pr view` triage, and the Phase 4 freshness re-check all draw on it too.

Before running the per-repo gate batch (or any of the Phase 3/4 batches above), check budget once: `gh api rate_limit --jq '.resources.search.remaining'`. If <15, poll (`until gh api rate_limit --jq '.resources.search.remaining' | awk '{exit !($1 > 15)}'; do sleep 15; done`) before fanning out. If the gate batch itself hits 403 (secondary rate limit), poll-and-retry only the failed queries — don't restart the whole batch.

## Phase 4 — Output

**Pre-output freshness re-check (BLOCKING).** Scout subagents may have run minutes ago and a PR can land in that window — your shortlist goes stale. Before emitting the ranked list, run in a single batched message, per top candidate: (1) the issue-number token search, mandatory and all-states as in "Duplicate-PR search" above — never substitute a code-identifier paraphrase; (2) the Phase 3 enrichment GraphQL call (or its REST fallback), which also carries the timeline cross-refs. Apply the same drop rule as Phase 3, including the closed-not-merged triage buckets. Fold this batch into the Phase 3 rate-limit budget check below rather than treating it as free. Documented failures: 2026-05-13 — `mastra-ai/mastra#16514` passed the scout's dup-PR check; PR #16545 opened ~10 minutes later. 2026-06-09 — see the vitest case in "Duplicate-PR search" above; the same incident is why this re-check now searches all states and reuses the timeline instead of a code-identifier paraphrase. Both surfaced only when the user asked "did someone already do this?"
